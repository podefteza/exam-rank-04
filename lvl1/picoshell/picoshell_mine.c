...
int	picoshell(char **cmds[])
{
	...
	while (cmds[i])
	{
		if (cmds[i + 1] && pipe(fd) == -1) // pipe command
			return (close(fd_stdin), 1);
		else if (!cmds[i + 1]) // last command, no pipe
			fd[0] = fd[1] = -1;

		pid = fork();
		if (pid == 0)
		{
			if (fd_stdin != 0 && dup2(fd_stdin, STDIN_FILENO) == -1) // connect previous command's output to this command's STDIN
				exit(1);
			if (fd[1] != -1 && dup2(fd[1], STDOUT_FILENO) == -1) // connect this command's STDOUT to the pipe for next command
				exit(1);
		...
		if (fd[1] != -1)
			close(fd[1]); // close write end (child uses it)
